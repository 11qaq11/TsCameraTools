...
router.post('/login', async (_req: Request, res: Response) => {
  ...
  try {
    ...
    const { rows } = await query(
      `INSERT INTO users (feishu_id, name, email, tenant_key, role)
       VALUES ('debug', '调试用户', 'debug@local', 'debug', 'owner')
       ON CONFLICT (feishu_id) DO UPDATE SET last_login_at = NOW(), role = 'owner'
       RETURNING id`,
    )
    ...
    res.json({
      token: sessionId,
      user: { id: userId, name: '调试用户', email: 'debug@local', feishu_id: 'debug', tenant_key: 'debug', role: 'owner' }
    })
  } catch (e) {
