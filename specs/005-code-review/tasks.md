...
- [ ] T001 [P] 审查 src/App.tsx - 路由配置、ProtectedRoute、懒加载、认证检查
- [ ] T002 [P] 审查 src/main.tsx - Redux Provider、HashRouter、根组件渲染
- [ ] T003 审查 src/pages/Devices.tsx - ShellPanel 逻辑、ADB 命令、终端事件、中文输入、命令历史、搜索功能
- [ ] T004 [P] 审查 src/pages/DevicesWeb.tsx - Socket 通信、ADB API、设备列表、错误处理
- [ ] T005 [P] 审查 src/pages/Login.tsx - 飞书 OAuth、Token 存储、错误处理
- [ ] T006 [P] 审查 src/pages/AuthCallback.tsx - OAuth 回调处理
- [ ] T007 审查 src/components/terminal/Term.tsx - xterm.js 初始化、Addon 加载、事件监听、资源清理
- [ ] T008 审查 src/components/terminal/HyperTerminal.tsx - 组件组合、Socket 事件、终端状态
- [ ] T009 [P] 审查 src/components/terminal/SearchBox.tsx - 搜索功能逻辑
- [ ] T010 [P] 审查 src/components/terminal/StatusBar.tsx - 状态显示逻辑
- [ ] T011 [P] 审查 src/components/terminal/Header.tsx - 终端头部组件
- [ ] T012 [P] 审查 src/components/Sidebar.tsx - 侧边栏导航
- [ ] T013 [P] 审查 src/components/Header.tsx - 顶部栏组件
- [ ] T014 [P] 审查 src/components/ErrorBoundary.tsx - 错误边界处理
- [ ] T015 [P] 审查 src/components/LogViewer.tsx - 日志查看器

### Phase 1.2: 前端 Hooks

- [ ] T016 [P] 审查 src/hooks/useAuth.ts - 认证状态、Token 刷新、登出处理
- [ ] T017 [P] 审查 src/hooks/useSocket.ts - Socket 连接、重连逻辑、事件处理
- [ ] T018 [P] 审查 src/hooks/useTerminal.ts - 终端生命周期、数据发送、状态管理
- [ ] T019 [P] 审查 src/hooks/useSearch.ts - 搜索功能逻辑

### Phase 1.3: 前端工具和配置

- [ ] T020 [P] 审查 src/utils/auth.ts - Token 管理、API 请求封装
- [ ] T021 [P] 审查 src/utils/logger.ts - 日志工具、错误处理
- [ ] T022 [P] 审查 src/store/index.ts - Redux Store 配置
- [ ] T023 [P] 审查 src/store/reducers/ui.ts - UI 状态管理
- [ ] T024 [P] 审查 src/store/reducers/sessions.ts - 会话状态管理
- [ ] T025 [P] 审查 src/config/default.ts - 默认配置
- [ ] T026 [P] 审查 src/types/index.ts - 类型定义完整性
- [ ] T027 [P] 审查 src/types/hyper.ts - Hyper 风格类型定义
- [ ] T028 [P] 审查 src/layouts/MainLayout.tsx - 主布局组件

### Phase 1.4: 后端模块

- [ ] T029 审查 server/index.ts - Express 配置、Socket.io 集成、中间件
- [ ] T030 [P] 审查 server/config.ts - 配置管理、环境变量
- [ ] T031 审查 server/routes/auth.ts - OAuth 流程、Token 验证
- [ ] T032 审查 server/routes/adb.ts - ADB 命令执行、设备列表
- [ ] T033 [P] 审查 server/routes/logs.ts - 日志 API
- [ ] T034 审查 server/services/shell.ts - Shell 进程管理、Socket 事件
- [ ] T035 [P] 审查 server/types/index.ts - 类型定义

### Phase 1.5: Electron 模块

- [ ] T036 审查 electron/main.cjs - 窗口创建、IPC 处理、ADB 功能
- [ ] T037 [P] 审查 electron/preload.cjs - API 暴露、安全性

### Phase 1.6: 配置文件

- [ ] T038 [P] 审查 package.json - 依赖版本、脚本配置
- [ ] T039 [P] 审查 vite.config.ts - Vite 配置、构建优化
- [ ] T040 [P] 审查 tsconfig.json - TypeScript 配置
- [ ] T041 [P] 审查 .oxlintrc.json - Lint 配置
