...
- [ ] T008 [P] 测试 BUG-001 - 启动程序，连接设备，执行 `ls` 命令，验证输出每行前有空格
- [ ] T009 [P] 测试 BUG-002 - 启动程序，连接设备，输入 `abc` 后 Ctrl+C，再输入 `ls`，验证执行正常
- [ ] T010 [P] 测试 BUG-003 - 启动程序，连接设备，执行 `echo hello`，选中文本，在记事本粘贴验证
- [ ] T011 [P] 测试 BUG-004 - 启动程序，连接设备，执行 `ls -la /system`，验证最后一行始终可见
- [ ] T012 回归测试 - 验证历史命令、搜索功能、中文输入等现有功能不受影响
