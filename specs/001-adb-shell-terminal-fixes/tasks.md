...
- [x] T023 [P] 测试 BUG-001 - 启动程序，连接设备，截图验证空格
- [x] T024 [P] 测试 BUG-002 - 启动程序，查看 `logs/` 目录日志，验证 ADB 检测
- [x] T025 [P] 测试 BUG-003 - 使用微软拼音输入法输入中文，验证显示正确
- [x] T026 [P] 测试 BUG-004 - 调整窗口大小，验证终端高度自适应
- [x] T027 [P] 测试 FEAT-001 - 执行多条命令，验证上下方向键历史导航
- [x] T028 [P] 测试 FEAT-002 - 按 Ctrl+R，输入关键词，验证历史搜索
- [x] T029 [P] 测试持久化 - 重启程序，验证历史记录是否保留

### 性能测试

- [x] T030 [P] 测试 NFR-001 - 验证 ADB 检测在 2 秒内完成
- [x] T031 [P] 测试 NFR-001 - 验证终端输入响应延迟 < 100ms

### 兼容性测试

- [x] T032 [P] 测试 NFR-002 - 在 Windows 10 上测试所有功能
- [x] T033 [P] 测试 NFR-002 - 在 Windows 11 上测试所有功能
- [x] T034 [P] 测试 NFR-002 - 使用微软拼音输入法测试中文输入
- [x] T035 [P] 测试 NFR-002 - 使用搜狗输入法测试中文输入

### 集成测试

- [x] T036 端到端测试 - 完整流程：启动 → 检测 ADB → 连接设备 → 执行命令 → 搜索历史 → 重启验证
